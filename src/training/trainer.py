...
def train_model(
    ...
    final_metrics = compute_metrics(all_val_true, all_val_pred, task_type)
...
def hierarchical_training_pipeline(
    ...
    level_0_train_losses, level_0_val_losses, level_0_metrics = train_model(
        level_0_model, train_loader_ret, val_loader_ret, device=device, task_type="regression"
    )
    ...
    level_1_train_losses, level_1_val_losses, level_1_metrics = train_model(
        level_1_model, train_loader_ret, val_loader_ret, device=device, task_type="regression"
    )
    ...
    level_2_train_losses, level_2_val_losses, level_2_metrics = train_model(
        level_2_model, train_loader_dir, val_loader_dir, device=device, task_type="classification"
    )
...
def run_experiment(params: dict, gpu_id: Optional[int] = None) -> dict:
    """Main experiment function for the orchestrator."""
    # Set device
    ...
    X, y_ret, y_dir, sample_weights = preprocessor.prepare_data(data)
    
    # Create data loaders
    train_dataset = TensorDataset(X, y_ret)
