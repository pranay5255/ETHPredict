...
| **Lighter** | ETH perp/spot OHLCV, funding, order book snapshots | 1m-1d | API pull |
...
Collect public Lighter ETH data into the existing CSV training layout:

```bash
python scripts/lighter_collect_data.py --config configs/config.yml
```

This writes Binance-compatible OHLCV rows under `data/raw/` and Lighter-native side data under `data/lighter/`. See `context/lighter_data_setup.md` for endpoint coverage and output files.
...
data:
  sources: [lighter, binance, defillama, santiment]
